    ...
    <div style={{ position: 'relative' }}>
      ...
      {tempPassword && selectedUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#0D1117', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: '2rem', width: 440, maxWidth: '90vw' }}>
            <div style={{ fontWeight: 700, fontSize: '1.05rem', marginBottom: '0.35rem' }}>Password temporanea generata</div>
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.82rem', marginBottom: '1.25rem' }}>
              Utente: <span style={{ color: '#fff', fontWeight: 600 }}>{selectedUser.name}</span> ({selectedUser.email})
            </div>

            <div style={{ background: 'rgba(124,58,237,0.12)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
              <code style={{ fontFamily: 'monospace', fontSize: '1.1rem', fontWeight: 700, color: '#a78bfa', letterSpacing: '0.08em', wordBreak: 'break-all' }}>
                {tempPassword}
              </code>
              <button
                style={{ ...btn('outline'), flexShrink: 0, fontSize: '0.72rem' }}
            ...
            <div style={{ background: 'rgba(234,179,8,0.08)', border: '1px solid rgba(234,179,8,0.2)', borderRadius: 8, padding: '0.75rem 1rem', marginBottom: '1.5rem', fontSize: '0.82rem', color: 'rgba(234,179,8,0.9)', lineHeight: 1.5 }}>
              Comunica questa password al negoziante e digli di cambiarla appena effettua l&apos;accesso, dalle Impostazioni del proprio account.
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button style={btn('primary')} onClick={() => { setTempPassword(null); setSelectedUser(null) }}>
