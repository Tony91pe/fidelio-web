...
let customers
if (segment==='atrisk') customers = await db.customer.findMany({where:{shopId:shop.id,lastVisitAt:{lt:
ago30}}})
else if (segment==='active') customers = await db.customer.findMany({where:{shopId:shop.id,lastVisitAt
:{gte:ago30}}})
...
to: c.email, subject,
html: '<div style="font-family:sans-serif;padding:2rem"><p>Ciao ' + c.name + '!</p>' +
'<div>' + body + '</div>' +
'<p style="color:#555;font-size:0.85rem;margin-top:2rem">Hai ' + c.points + ' punti da usare.<
/p></div>',
})
sent++
} catch {}
