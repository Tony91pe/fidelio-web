...
{id:'atrisk',label:'A rischio abbandono',desc:'Chi non viene da piu di 30 giorni'},
]
const templates = [
{subject:'Offerta speciale per te!',body:'Abbiamo una sorpresa per te. Vieni a trovarci!'},
{subject:'Ci manchi! Torna a trovarci',body:'E da un po che non ci vedi. I tuoi punti ti aspettano!'},
{subject:'Novita per te',body:'Abbiamo delle novita che non vediamo lora di condividere!'},
]
...
Invia email ai tuoi clienti per farli tornare
</p>
{result && (
...
{result.sent} email inviate su {result.total} clienti!
</p>
<button onClick={()=>setResult(null)}
style={{marginLeft:'auto',background:'transparent',border:'none',
color:'rgba(255,255,255,0.4)',cursor:'pointer',fontSize:'1.2rem'}}>x</button>
</div>
)}
...
style={{background:segment===s.id?'rgba(108,61,244,0.2)':'rgba(255,255,255,0.04)',
border:'1px solid '+(segment===s.id?'rgba(108,61,244,0.4)':'rgba(255,255,255,0.08)'),
borderRadius:'12px',padding:'0.8rem 1rem',cursor:'pointer',marginBottom:'0.6rem'}}>
<div style={{fontWeight:'600',fontSize:'0.9rem'}}>{s.label}</div>
<div style={{fontSize:'0.75rem',color:'rgba(255,255,255,0.4)',marginTop:'0.2rem'}}>{s.desc
}</div>
</div>
))}
<h3 style={{fontWeight:'700',margin:'1rem 0 0.5rem',fontSize:'0.95rem'}}>2. Template pronti</h
3>
{templates.map((t,i) => (
<button key={i} onClick={()=>{setSubject(t.subject);setBody(t.body)}}
style={{display:'block',width:'100%',background:'rgba(255,255,255,0.04)',
border:'1px solid rgba(255,255,255,0.08)',borderRadius:'10px',
padding:'0.6rem 1rem',cursor:'pointer',color:'white',
textAlign:'left',fontSize:'0.85rem',marginBottom:'0.4rem'}}>
{t.subject}
</button>
))}
</div>
...
<input style={inp} placeholder="Oggetto email"
value={subject} onChange={e=>setSubject(e.target.value)} required />
<textarea style={{...inp,height:'150px',resize:'vertical'} as any}
placeholder="Testo email..."
value={body} onChange={e=>setBody(e.target.value)} required />
<button type="submit" disabled={sending}
style={{width:'100%',background:'#6C3DF4',color:'white',padding:'12px',
