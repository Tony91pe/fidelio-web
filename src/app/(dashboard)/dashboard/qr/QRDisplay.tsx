...
export default function QRDisplay({
  shopId, shopName, checkinUrl
}: {
  shopId: string; shopName: string; checkinUrl: string
}) {
  return (
    <div style={{display:'grid', gridTemplateColumns:'auto 1fr', gap:'2rem', alignItems:'start'}}>
      ...
      <div style={{background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)',
        ...
        <p style={{fontSize:'0.85rem', color:'rgba(255,255,255,0.6)', lineHeight:'1.8'}}>
          Stampa questa pagina o mostra lo schermo al cliente.
          Il cliente inquadra il QR con la fotocamera, inserisce nome e email
          e riceve 50 punti di benvenuto automaticamente.
        </p>
        <p style={{fontSize:'0.75rem', color:'rgba(255,255,255,0.3)', marginTop:'1rem', wordBreak:'break-all'}}>
          URL: {checkinUrl}
        </p>
        <button onClick={() => window.print()}
          style={{marginTop:'1rem', background:'#6C3DF4', color:'white',
            padding:'10px 20px', borderRadius:'10px', border:'none', cursor:'pointer', fontWeight:'600'}}>
          Stampa QR code
        </button>
      </div>
    </div>
